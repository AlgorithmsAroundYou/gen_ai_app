...
llm = GoogleGenerativeAI(model = "models/text-bison-001", google_api_key="AIzaSyD1ZwHiBzaARkdmLVI9lD1UwRzdKJDtVcI", temperature=0.5)
...
def text_to_ai(data, language_code): 
    ...
    instructor_embedding = HuggingFaceInstructEmbeddings(model_name = "hku-nlp/instructor-large")
    print("instructor_embedding ------------", instructor_embedding)
...
def video_summarizer() -> None:
    #st.set_page_config(page_title="Video Summarizer Demo", page_icon="📹")
    st.markdown("# Video Summarizer Demo")
    st.header("Video Summarizer Demo")
    st.write(
    ...
    link = st.text_input('Enter video you want to summarize:', testVideo)
    language_code = 0 
    #st_player(link)
    st.video(link)

    if st.button('Start Load'):
        if link:
            try:       
                ...
                st.text('Loading the transcript...')
                ...
                st.text(f'AI generated summary, click on Summarize button to View')
